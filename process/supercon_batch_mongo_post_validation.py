...
from process.supercon_batch_mongo_compute_table import MongoTabularProcessor
from supercon_batch_mongo_extraction import connect_mongo, MongoSuperconProcessor
from utils import json_serial


class MongoPostValidationProcessor(MongoSuperconProcessor):
    grobid_client = None
    config = {}

    def __init__(self, config_path, force=False, verbose=False):
        super(MongoPostValidationProcessor, self).__init__(config_path, verbose)
        self.force = force
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-threads", "-n", help="Number of concurrent processes", type=int, default=50,
                        required=False)
    parser.add_argument("--database", "-db",
                        help="Set the database name which is normally read from the configuration file", type=str,
                        required=False)
    parser.add_argument("--force", "-f", help="Re-process all the records", action="store_true", default=False)
